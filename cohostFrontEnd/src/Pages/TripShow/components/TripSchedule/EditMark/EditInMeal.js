...
const EditInMealForm = (props) =>{
    ...
    const [list, setList] = useState(markData.products);
    ...
    const onSubmit = async() => {
        ...
        if(isValid.valid){
            dispatch(editMark({tripId, day, mark:{mark:'inMeal', data, markId:markData.id}}));
    ...
    return(
        <div id='EditInMealForm'>
            <form className="mark-form">
                <h3>{currentTripLength[0]}</h3>
                <div>
                    <input className="mark-input IM" type="text" defaultValue={markData.name} placeholder="Name" onChange={(e)=>{setName(e.target.value)}}/>
                    <div className="mark-container">
                        <input className="mark-input IM" type="time" defaultValue={markData.startTime} name="" id="" onChange={(e)=>{setStartTime(e.target.value)}}/>
                        <span className="time-label">to</span>
                        <input className="mark-input IM" type="time" defaultValue={markData.endTime} name="" id="" onChange={(e)=>{setEndTime(e.target.value)}}/>
                    </div>
                </div>
            </form>
            <div className="shopping">
                <div className="search">
                    ...
                    {onCart === 1 &&
                        <div id="list-data">
                            <button className="mark-success IM" onClick={()=>{onSubmit()}}>Save</button>
                            {list && list.length>0 &&
                                <div className="products">
                                    <div className="products-box">
                                        {list.map((product)=>
                            ...
                            <button className="mark-delete IM" onClick={()=>{killMark()}}>Delete Meal</button>
                        </div>
                    }
                </div>
            </div>
